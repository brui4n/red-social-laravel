...
<div class="max-w-4xl mx-auto px-4 py-8 space-y-8">
    ...
    <div class="bg-white p-6 rounded-2xl shadow border border-gray-200">
        ...
        @if($post->imagen)
            <div class="text-center my-6">
                <img src="{{ asset('storage/' . $post->imagen) }}" alt="Imagen del post"
                     class="rounded-xl shadow max-h-96 w-full object-contain border">
            </div>
        @endif

        @if($post->codigo)
            <div class="bg-gray-900 text-green-200 text-sm font-mono rounded-lg p-4 overflow-x-auto mb-4">
                <pre><code>{{ $post->codigo }}</code></pre>
            </div>
        @endif

        @if($post->lenguaje)
            <p class="text-sm text-gray-500"><strong>Lenguaje:</strong> {{ $post->lenguaje }}</p>
        @endif
        ...
        <div class="flex flex-wrap gap-4 mt-6">
            ...
            <button onclick="copiarEnlace()"
                    class="bg-green-100 text-green-700 px-4 py-2 rounded-full hover:bg-green-200 transition">
                📤 Compartir
            </button>
        </div>
    </div>
    ...
    {{-- 💬 Comentarios --}}
    <div class="bg-white p-6 rounded-2xl shadow border border-gray-200">
        <h3 class="text-2xl font-semibold mb-6 text-gray-800">🗨️ Comentarios</h3>
