            ...
            @foreach($posts as $post)
                <div class="bg-white shadow-md rounded-xl p-5 border border-gray-200">
                    ...
                    <p class="text-sm text-gray-500 mb-2">
                        Publicado por 
                        @if($post->usuario)
                            <a href="{{ route('profile.show', $post->usuario) }}" 
                               class="font-medium text-blue-600 hover:text-blue-800 hover:underline transition cursor-pointer">
                                {{ $post->usuario->nombre }}
                            </a>
                        @else
                            <span class="font-medium text-gray-700">Usuario desconocido</span>
                        @endif
                    </p>
